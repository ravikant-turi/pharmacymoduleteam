...
<div class="top">
    ...
    <h:form id="searchForm">
        <h:panelGrid columns="4" cellpadding="5">
            ...
            <h:commandButton value="Search" action="#{viewController.searchMedicines}" />
    ...
    <h:form id="tableForm">
        <h:panelGroup rendered="#{not empty viewController.medicinesList}">
            <h:dataTable value="#{viewController.paginatedMedicines}" var="med" border="1" styleClass="table">
                ...
                <h:column>
                    <f:facet name="header">
                        <h:commandLink value="Name #{viewController.sortBy eq 'medicineName' ? (viewController.ascending ? '↑' : '↓') : ''}"
                ...
                <h:column>
                    <f:facet name="header">
                        <h:commandLink value="Price #{viewController.sortBy eq 'unitPrice' ? (viewController.ascending ? '↑' : '↓') : ''}"
...
<h:column>
    <f:facet name="header">
        <h:commandLink value="Expiry #{viewController.sortBy eq 'expiryDate' ? (viewController.ascending ? '↑' : '↓') : ''}"
...
<h:outputText value="#{med.expiryDate}" 
    ...
    <f:convertDateTime pattern="dd-MM-yyyy" timeZone="Asia/Kolkata"/>
            ...
            <div style="text-align:center; margin-top: 20px;">
                <h:commandButton value="<-Prev" action="#{viewController.previousPage}" disabled="#{viewController.currentPage == 1}" />
                <h:outputText value=" Page #{viewController.currentPage} of #{viewController.totalPages} " style="margin: 0 10px;" />
                <h:commandButton value="Next ->;" action="#{viewController.nextPage}" disabled="#{viewController.currentPage == viewController.totalPages}" />
            </div>
        </h:panelGroup>
